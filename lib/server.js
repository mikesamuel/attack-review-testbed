...
function start({ hostName, port, rootDir, dbConfig }, onStart, unguessable = mintUnguessable) {
  const staticFileRoot = path.join(rootDir, 'static');
  const staticHandler = serveStatic(staticFileRoot, { setHeaders });

  const database = new safepg.Pool(dbConfig);
  ...
  function handleRequest(req, res) {
    ...
    res.setHeader(
      ...
      'Content-Security-Policy',
      `default-src none; img-src 'self'; form-action 'self'; script-src 'nonce-${
        cspNonce }'; report-uri /client-error`);
    ...
    try {
      ...
      handle(bundle, handleError.bind(null, bundle));
      // handlers are responsible for calling res.end().
    } catch (exc) {
      handleError(bundle, exc);
    }
  }
}
